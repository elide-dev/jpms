...
export class ClassFile {
  ...
  // nothing yet
  private constructor(private readonly buf: ByteBuffer) {
    if (buf.readUint32() !== 0xcafebabe) throw Error("Invalid MAGIC value");

    this.minor_version = buf.readUint16();
    this.major_version = buf.readUint16();
    this.constant_pool_count = buf.readUint16();
    this.constant_pool = this._readConstantPool(this.constant_pool_count - 1);
    ...
    this.interfaces = this._readInterfaces(this.interfaces_count);
    this.fields_count = buf.readUint16();
    this.fields = this._readMemberInfoArray(this.fields_count);
    this.methods_count = buf.readUint16();
    this.methods = this._readMemberInfoArray(this.methods_count);
    this.attributes_count = buf.readUint16();
    this.attributes = this._readAttributeInfoArray(this.attributes_count);
  }
   ...
   */
  _readMemberInfoArray(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      memberInfo.attributes = this._readAttributeInfoArray(memberInfo.attributes_count as number);
      members[i] = memberInfo;
    }
    return members;
  }

  _readAttributeInfoArray(attributes_count: number) {
  ...
  _readAttributeInfo() {
    ...
    const attributeNameBytes = this.constant_pool[attribute.attribute_name_index].bytes;
    ...
    switch (attributeName) {
      ...
      case "RuntimeVisibleParameterAnnotations": {
        ...
        for (let parameterIndex = 0; parameterIndex < attribute.num_parameters; parameterIndex++) {
          const parameter_annotation: any = { num_annotations: this.buf.readUint16() };
  ...
  _readStackMapAttribute(attribute) {
    ...
    for (let entryIndex = 0; entryIndex < attribute.number_of_entries; entryIndex++) {
      const stack_map_frame: any = { offset: this.buf.readUint16() };
