...
export class ToolError extends Error {
  private constructor(public readonly run: ToolRun, message?: string) {
  ...
  private static explainErr(run: ToolRun): string {
    return [`${run.exec.bin.name} failed with exit code ${run.exitCode}
=================================================================================
Working Directory:
  ${run.cwd}
...
Environment:
---------------------------------------------------------------------------------
${Object.entries(run.env || {}).map(([k, v]) => `- ${k} = ${v}`).join('\n')}
`, `
Standard Error:
---------------------------------------------------------------------------------
${run.result.stderr}
